...
The runtime matrix comes from `.ci/matrix.json`. It covers PHP 8.2–8.5 on `ubuntu-24.04` x64, `windows-2022` x64, and `macos-15` Arm64 in three extension modes. The scheduled matrix adds Ubuntu Arm64, genuine Windows x86, Windows x64 thread-safe builds, PHP 8.6 early-warning coverage, and the selected ICU boundaries. The release workflow reruns all evidence and tests the exact Composer archive.
...
1. Move `.github/ci/public-pull-request.yml`, `.github/ci/public-scheduled.yml`, and `.github/ci/public-release.yml` into `.github/workflows/`.
