...
Until the upstream-derived Test262 roadmap supplies complete spec coverage, pull-request CI treats only the reviewed spec mutation baseline in `.ci/spec-mutation-expected-failure.json` as an expected failure. The individual job records its raw failed outcome, and the aggregate command accepts it only while every other campaign passes, every spec mode meets or improves its baseline score and failure count, and no failure category exceeds its reviewed maximum. A mutation regression, missing or malformed evidence, or an unexpectedly passing spec campaign makes CI red; the latter is the canary requiring removal of the `--expect-failing` option and the step-level handling.
