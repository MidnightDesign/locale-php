...
`tools/merge-mutation-reports.php` compares the complete mutant identity multiset across absent, disabled, and native extension modes. Each mutant applies to all modes unless its stable campaign-and-mutant identity has an explicit narrower entry in `.ci/mutation-applicability.json`; stale or invalid entries fail aggregation. It records every applicable per-mode obligation in `build/matrix-mutation-score.json` and fails unless every obligation is killed by its attributed test suite. Static-analysis detections do not receive test-kill credit. Escaped, uncovered, timed-out, errored, syntax-error, ignored, skipped, missing, malformed, or inconsistent evidence fails the gate; aggregation failures are themselves retained as machine-readable evidence.
