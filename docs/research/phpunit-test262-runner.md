...
Locale's runner is stricter than the Calendrics prior art. Before yielding any tests, it compares discovered scripts with the canonical execution manifest in translated conformance evidence, rejects duplicate script paths or PHPUnit identities, and rejects missing or stale scripts. Multiple scripts may retain one source fixture when meaningful PHP representation variants require distinct PHPUnit identities. Generated syntax errors and escaping spec-layer errors remain PHPUnit errors rather than becoming incomplete evidence.
