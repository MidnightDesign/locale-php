...
class Locale
{
    ...
    private static function readOption(array|object $options, string $name): OptionValue
    {
        if ($options instanceof OptionBag) {
            return $options->has($name) ? OptionValue::present($options->get($name)) : OptionValue::missing();
        }

        if (is_array($options)) {
            return array_key_exists($name, $options) ? OptionValue::present($options[$name]) : OptionValue::missing();
        }

        $properties = get_object_vars($options);

        return array_key_exists($name, $properties) ? OptionValue::present($properties[$name]) : OptionValue::missing();
    }
