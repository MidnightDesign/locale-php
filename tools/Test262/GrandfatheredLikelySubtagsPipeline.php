...
final class GrandfatheredLikelySubtagsPipeline implements FixturePipeline
{
    ...
    public function run(string $source, string $fixturePath): FixtureResult
    {
        ...
        return new FixtureResult(
            ...
            array_values(array_map(static fn (array $identity): array => [
                ...$identity,
                'status' => $status,
                'adaptations' => ['JavaScript loops and defaulted record fields are expanded into named PHPUnit data sets.'],
            ], $identities)),
    ...
    private static function regularTags(string $source): array
    {
        ...
        foreach ($matches as $match) {
            preg_match_all('/(?<key>tag|canonical|maximized|minimized)\s*:\s*"(?<value>[^"]+)"/', $match['record'], $fields, PREG_SET_ORDER);
            $record = [];
    ...
    {
        return (int) ($locale->toString() !== $canonical)
...
PHP."\n";
    ...
    private static function export(array $value): string
    {
        return preg_replace('/[ \t]+$/m', '', PhpExporter::export($value))
            ?? throw new \RuntimeException('Unable to format grandfathered likely-subtag cases.');
    }
}
