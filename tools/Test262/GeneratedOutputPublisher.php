...
final class GeneratedOutputPublisher
{
    ...
    public function publish(array $outputs): void
    {
        ...
        try {
            ...
            try {
                $generatedBackedUp = $this->moveIfPresent($test262Root . '/Generated', $backupRoot . '/Generated');
    ...
    private function validate(array $outputs): void
    {
        ...
        foreach (array_keys($outputs) as $path) {
            if ($path !== 'tests/Test262/evidence.json' && !GeneratedScript::isGeneratedPath($path)) {
                throw new \InvalidArgumentException(
                    'Test262 publication contains an invalid output path: ' . $path . '.',
                );
            }
        }
    }
