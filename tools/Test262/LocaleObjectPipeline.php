...
final class LocaleObjectPipeline implements FixturePipeline
{
    ...
    public function run(string $source, string $fixturePath): FixtureResult
    {
        ...
        if (count($assertions) !== 6) {
            return FixtureResult::translationGap($fixturePath, $source, ['associative_array', 'plain_object'], $assertions, new TranslationGap('Expected six existing-Locale assertions.'));
        }
        $checks = self::checks();
        $evidence = [];
        foreach ($assertions as $index => $assertion) {
            $passing = $checks[$index];
            $evidence[] = [...$assertion, 'status' => $passing ? 'passing' : 'failing', 'adaptations' => ['The initialized spec-layer Locale directly represents the JavaScript Intl.Locale input object.', 'Override options run with associative-array and plain-object representations where meaningful.'], 'executions' => [['id' => 'assertion-'.($index + 1), 'assertionId' => $assertion['id'], 'representation' => $index % 2 === 1 ? 'associative_array_and_plain_object' : 'locale_object', 'status' => $passing ? 'passing' : 'failing']]];
        }
        $failureCount = count(array_filter($checks, static fn (bool $passing): bool => !$passing));

        return new FixtureResult($fixturePath, hash('sha256', $source), $failureCount === 0 ? 'passing' : 'failing', ['associative_array', 'plain_object'], $evidence, 6, $failureCount, ['tests/Test262/Generated/ConstructorLocaleObjectTest.php' => $this->render($fixturePath)]);
    }
    ...
    private static function checks(): array
    {
        ...
        return [$enUS->toString() === 'en-US', $enGB->toString() === 'en-GB', $zhUnihan->toString() === 'zh-u-co-unihan', $zhZhuyin->toString() === 'zh-u-co-zhuyin', $zhUnihan->collation === 'unihan', $zhZhuyin->collation === 'zhuyin'];
    }
