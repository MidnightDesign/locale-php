...
if ($argc !== 5) {
    fwrite(
        STDERR,
        "Usage: php tools/record-ci-provenance.php <output> <requested-php> <runner-label> <extension-mode>\n",
    );
    exit(2);
}
...
if ($tracePath === false || !is_file($tracePath)) {
    throw new RuntimeException(
        'The branch trace does not exist. Run the test suite with CI bootstrap variables first.',
    );
}
